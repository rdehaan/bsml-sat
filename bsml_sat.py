...
def formula_to_asp_facts(formula):
    ...
    index, program = fact_constructor(1, formula)
...
def solve_bsml_sat(
    ...
    model_program += """
        ...
        1 { state_edge(N,M) : state_node(M), N != M } :- state_type(N,join), state_nonempty(N).
        1 { state_edge(N,M) : state_node(M), N != M } :- state_type(N,relation), state_nonempty(N).
        1 { state_edge(N,M) : state_node(M), N != M } :- state_type(N,full_relation), state_nonempty(N).
        ...
        1 { state_successor(M,W) : state_edge(N,M) } 1 :- state_type(N,relation), state(N,W).
        :- state_successor(M,W1), state(M,W2), not relation(W1,W2).
        1 { state_full_successor(M,W) : state_edge(N,M) } 1 :- state_type(N,full_relation), state(N,W).
        :- state_full_successor(M,W1), state(M,W2), not relation(W1,W2).
        :- state_full_successor(M,W1), relation(W1,W2), not state(M,W2).
        :- state_node(N), not state_root(N), not state_edge(M,N) : state_node(M); not state_type(N,inactive).
        ...
        :- state_edge(N,M), M <= N.
        :- state_node(N), state_node(M), state_type(N,inactive), not state_type(M,inactive).
